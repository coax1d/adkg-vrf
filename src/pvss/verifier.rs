...
impl<C: Pairing> crate::pvss::Params<C> {
    pub fn verify_transcript_unoptimized<R: Rng>(
        &self,
        ss: &SecretSharingWithWitness<C>,
        rng: &mut R,
    ) {
        ...
        assert_eq!(
            C::pairing(ss.payload.h1, self.config.g2),
            C::pairing(self.config.g1, ss.payload.h2)
        );
    ...
    fn verify_bgpks<R: Rng>(&self, ss: &SecretSharingWithWitness<C>, rng: &mut R) {
        ...
        let lis_at_z =
            BarycentricDomain::of_size(self.config.domain, self.config.n).lagrange_basis_at(z);
    ...
    fn verify_as<R: Rng>(&self, t: &SecretSharingWithWitness<C>, rng: &mut R) {
        ...
        let ls_deg_n_at_z =
            BarycentricDomain::of_size(self.config.domain, self.config.n).lagrange_basis_at(z);
        let ls_deg_t_at_z =
            BarycentricDomain::of_size(self.config.domain, self.config.t).lagrange_basis_at(z);
    ...
    fn verify_c(&self, ss: &SecretSharingWithWitness<C>) {
        ...
        let ls_at_0 = BarycentricDomain::of_size(self.config.domain, self.config.t)
